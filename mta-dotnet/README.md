...
> Click in the terminal window and hit Enter to initialize the session.
...
Switch to the source directory and check out the `part-1` branch, which has the first version of the app::
...
In the `builder` stage, the Dockerfile copies the source code into the image and just runs the existing <a href="https://github.com/dockersamples/mta-netfx-dev/blob/part-1/src/SignUp/SignUp.Web/build.ps1" target="_blank">build.ps1</a>) script. When this stage completes, the output is a published website folder, which is available for later stages to use.
...
> The app is running fine in Docker, with no code changes from the original ASP.NET 3.5 codebase. The Dockerfile has all the logic to build and package the app, so any developer and all the CI servers can build and run the app from the source - the only dependency is Docker.
...
Switch to the `part-3` branch which has the new version of the app, and builds it using Docker Compose:
...
It's easy to add functionality to your app when it's running in Docker. This version of the app is using <a href="http://nats.io" target="_blank">NATS</a>, an open-source production-grade message queue. The NATS team publish an official Docker image which means you can add messages to your app just by running a container.

> This version of the app does scale. You could run dozens of web containers and just a handful of message handler containers. Under high load, the message queue will retain messages until the handlers have the capacity to process them - and SQL Server is no longer a bottleneck.
...
> The app has a modern architecture now, powered by Docker and without needing a full rewrite. You've extracted key features from the app and run them in a separate container, using Docker to plug everything together and to give you a consistent build and deployment process for the whole solution.
...
Open the [compose file](./docker-stack.yml) and copy the contents to the clipboard. Then paste the YAML into the UCP _Ceate Stack_ and update the image name for the app server.
...
Here there are 9 containers running as swarm services for my application, and with Docker EE you can also run Linux containers as a Kubernetes stack. You use DTR to ship, scan and sign all your images, and UCP to manage all your apps - a consistent platform for any type of application.
